...
void DSCoreLib::getCVData(word inAddress, US *inMaxData, US *inMinData, byte inD, byte inK, byte inWaitTime, byte inCntReset, byte inCntCV, word inAdcOffset)
{
	...
	for( byte i = 0; i < inCntCV; i++)
	{
		...
		/* CV Ack待ち時間を2回目以降は短縮する。応答しない可能性の方が多いため。 */
		if( i == 0)
		{
			//aWaitTime = (inWaitTime >> 1);
		}
		else
		{
			if( aWaitTime > 2)
...
{
	//この関数は100us程度かかる
	long aCurrent = analogRead(PIN_CURRENT);
...
US DSCoreLib::GetCurrentAvg()
{
	...
	for( byte i = 0; i < 16; i++)
	{
		aCurrent = aCurrent + (long)analogRead(PIN_CURRENT);
	}
